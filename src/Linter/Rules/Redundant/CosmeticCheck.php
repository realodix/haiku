...
final class CosmeticCheck implements Rule
{
    ...
    private function findCandidates(array $entry, array $interactionMap): array
    {
        ...
        // Standard selector (non-attribute)
        ...
        // Find more general simple selectors by generating all subsets of components.
        if ($parsed !== null) {
            $components = [];
            if ($parsed['tag'] !== '') {
            ...
            for ($mask = 0; $mask < (1 << $n); $mask++) {
                ...
                if (isset($interactionMap[$subKey])) {
                    foreach ($interactionMap[$subKey] as $idx) {
                        ...
                        // Candidate should have parsedSimple not null, as we only put simple selectors into S bucket.
